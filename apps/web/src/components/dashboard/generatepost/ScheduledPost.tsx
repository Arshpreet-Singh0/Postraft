...
const ScheduleGeneartedPostModel = ({
  open,
  setOpen,
  ...
  useEffect(() => {
    if (open) {
      ...
      setContent(initialContent.slice(0,280));
      ...
      const localISOTime = new Date(localScheduled.getTime() - offsetMs)
        .toISOString()
        .slice(0, 16);
      ...
      const localMinISOTime = new Date(now.getTime() - nowOffsetMs)
        .toISOString()
        .slice(0, 16);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <p className="text-white bg-none hover:text-black">Edit Post</p>
      </DialogTrigger>
...
export default ScheduleGeneartedPostModel;
